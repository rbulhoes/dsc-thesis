...
deformation(matrix = as.matrix(S[,2:3]), # regi?o geogr?fica
            ...
            cex1 = 1.0,
            cex2 = 0.9)
legend("bottomleft", inset = .07, pch = c(25, 24), horiz = TRUE,
       cex = c(0.75, 0.75), pt.bg = c("goldenrod3", "darkolivegreen1"),
       legend = c(as.expression(bquote(italic(S))),
                  as.expression(bquote(italic(D)))))

deformation(matrix = as.matrix(S[,2:3]), # regi?o geogr?fica
            tarmatrix = as.matrix(TamanhoCem_stats[1:N,
                                                   c("Lon-Mean", "Lat-Mean")]),
            ngrid = 20,
            pch_S = 25,
            pch_D = 24,
...
deformation(matrix = as.matrix(S[,2:3]), # regi?o geogr?fica
            tarmatrix = as.matrix(TamanhoMil_stats[1:N,
                                                   c("Lon-Mean", "Lat-Mean")]),
            ngrid = 20,
            pch_S = 25,
            pch_D = 24,
