...
start = timer()
updt_D(it = 1, S_obs = S, M_X = X, M_Y = Y, Sigma_usage = Sigma,
       Beta_usage = Beta, V_usage = V, phi_usage = phi,
       D_usage = D, sigma2d_usage = sigma2d_usg, R_usage = R_usg)[:,2:4]
end = timer()
print(end - start)
...
if isfinite(psi_usage) is False:
  R_tot = np.identity(N_tot)
else:
  R_tot = nb_BR(matrix = S_tot,
                need_transp = True,
                squared = True,
                scalar = psi_usage)
R_gu = R_tot[0:N,N:N_tot]
R_1N = R_tot[0:N,0:N]
inv_R_1N = np.linalg.inv(R_1N)
R_star = R_tot[N:N_tot,N:N_tot]
D_cov = R_star - np.matmul(R_gu.T, inv_R_1N @ R_gu)

e_D_i = [np.zeros((2, N_i)) for k in range(0, smp_size)]
for k in range(0, int(smp_size)):
  D_mean_k = S_i + np.matmul(e_D[k] - S, inv_R_1N @ R_gu)
  e_D_i[k] = rMVN(n = 1,
                  avg = D_mean_k,
                  left = sigma2d_usg,
                  right = D_cov,
...
# Vectorization
l, m = 0, 0
vec = e_V[0]*e_Sigma[0][l,m]
for j in range(1, int(smp_size)):
  now = e_V[j]*e_Sigma[j][l,m]
  vec = np.concatenate((vec, now), axis = None)
...
for n in np.arange(2, N-1):
  for n_ in np.arange(n+1, N):
    aux = S[0,n]*S[1,n_] - S[0,n_]*S[1,n]
    if aux == 0:
      pass
    else:
      for j in np.arange(0, smp_size):
        now11 = (e_D[j][0,n]*S[1,n_] - e_D[j][0,n_]*S[1,n])/aux
        now21 = (e_D[j][1,n]*S[1,n_] - e_D[j][1,n_]*S[1,n])/aux
        now12 = (S[0,n]*e_D[j][0,n_] - S[0,n_]*e_D[j][0,n])/aux
        now22 = (S[0,n]*e_D[j][1,n_] - S[0,n_]*e_D[j][1,n])/aux
        vec11 = np.concatenate((vec11, now11), axis = None)
        vec21 = np.concatenate((vec21, now21), axis = None)
        vec12 = np.concatenate((vec12, now12), axis = None)
        vec22 = np.concatenate((vec22, now22), axis = None)
...
plt.hist(x = vec11,
         ...
         rwidth = 0.9)
plt.ylabel('Probability')
plt.title(r'$\Lambda_{1,1}$')
plt.axvline(x = Lambda[0,0],
            color = 'r',
            linestyle = '-')
...
plt.hist(x = vec12,
         ...
         rwidth = 0.9)
plt.ylabel('Probability')
plt.title(r'$\Lambda_{1,2}$')
plt.axvline(x = Lambda[0,1],
            color = 'r',
            linestyle = '-')
...
plt.hist(x = vec21,
         ...
         rwidth = 0.9)
plt.ylabel('Probability')
plt.title(r'$\Lambda_{2,1}$')
plt.axvline(x = Lambda[1,0],
            color = 'r',
            linestyle = '-')
...
plt.hist(x = vec22,
         ...
         rwidth = 0.9)
plt.ylabel('Probability')
plt.title(r'$\Lambda_{2,2}$')
plt.axvline(x = Lambda[1,1],
            color = 'r',
            linestyle = '-')

print(np.quantile(vec11, 0.025), np.quantile(vec11, 0.5), Lambda[0,0],
      np.mean(vec), np.quantile(vec11, 0.975))
print(np.quantile(vec12, 0.025), np.quantile(vec12, 0.5), Lambda[0,1],
      np.mean(vec), np.quantile(vec12, 0.975))
print(np.quantile(vec21, 0.025), np.quantile(vec21, 0.5), Lambda[1,0],
      np.mean(vec), np.quantile(vec21, 0.975))
print(np.quantile(vec22, 0.025), np.quantile(vec22, 0.5), Lambda[1,1],
      np.mean(vec), np.quantile(vec22, 0.975))
